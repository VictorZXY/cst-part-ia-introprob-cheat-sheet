...
%		A random variable $T=h(X_1,X_2,\cdots,X_n)$ depending only on the samples is called an \textit{estimator}.
%		
%		An \textit{estimate} $t=h(x_1,x_2,\cdots,x_n)$ is a value that only depends on the dataset.
...
%	\item Dice game (maximise the probability of stopping at the last 6 within $n$ throws):
%	
%	Optimal strategy: wait until there are 5 or 6 throws left, and then stop at the first 6
%	
%	Probability of success: $\left(\dfrac{5}{6}\right)^5$
